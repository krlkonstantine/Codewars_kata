...
//catch
//finally
//с помощью then мы достаем положительный результат
//then(res) получает результат res, прописанный в теле промиса
//с помощью catch мы дотаем ошибку catch(err), в теле пропса обозначенный rej
//в catch мы попадаем в случае режекта промиса
//then вообще принимает два коллбэка, один принимает res а другой err
//но обычно используют метод catch
//в метод finally мы попадаем всегда независимо от результата
//результат промиса содержит такой же промис
//поэтому после каждоо then можно писать еще один и еще один итд
//это называется цепочка промисов
...
//async сразу же возвращает Промис
//можно вместо создания промиса просто пометить функцию как async, и она вернет промис

//json placeholder публичный сервак для работы с запросами
